...
void LRFlip<SampleType>::prepare(const juce::dsp::ProcessSpec &spec) {
  reset();
}

template <typename SampleType>
void LRFlip<SampleType>::process(
    const juce::dsp::ProcessContextReplacing<SampleType> &context) noexcept {
...
template <typename SampleType>
void LRFlip<SampleType>::reset() noexcept {
  // reset
}
